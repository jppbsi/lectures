...
\justify Caso $f(n)\in O(g(n))$ e $d(n)\in O(e(n))$, então $f(n)d(n)\in O(g(n)e(n)))$.\newline
	\underline{Exemplo:} sejam $f(n)=n\in O(n)$ e $g(n)=n^2\in O(n^2)$. Temos que $f(n)g(n)=nn^2=n^3$. Neste caso, temos que $n^2\in O(n^3)$.
}
...
& $n^2\log(n)$ & $n(\log(n))^{10}$\\
 ...
 & $\underbrace{2\log(n)}_{\text{maior termo}}+\log(\log(n))$ & $\underbrace{\log(n)}_{\text{maior termo}}+10\log(\log(n))$\\
\end{tabular}
\end{table}
