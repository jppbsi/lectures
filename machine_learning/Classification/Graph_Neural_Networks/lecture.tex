...
\Sli{
Colocar figura\newline
\justifying Essencialmente, roteamento de mensagens e convolução são operações que \textbf{agregam} e \textbf{processam} informações de uma certa vizinhança para atualizar o valor de um determinado elemento. Em GNNs, cada elemento é um vértice; já em uma CNN cada elemento é um \emph{pixel}.\newline
}

\Sli{
\justifying Empilhando essas camadas GNN de roteamento de mensagens é uma maneira de fazer com que os nós recebam informações do grafo como um todo. Podemos, então, atualizar nossa arquitetura GNN da seguinte forma: FIGURA AQUI.
}
