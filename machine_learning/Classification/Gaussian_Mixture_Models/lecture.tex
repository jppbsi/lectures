...
\end{equation}
em que $\boldsymbol{\theta}=\{\theta_1,\theta_2,\ldots,\theta_K\}$, $\theta_k = (w_k,\boldsymbol{\mu}_k,\boldsymbol{\Sigma}_k)$ corresponde aos parâmetros relativos à Gaussiana $k$ e $\sum_{k=1}^kw_k=1$.
}
...
\Sli{
\justifying Seja ${\cal Z}=\{z_1,z_2,\ldots,z_m\}$ um conjunto de variáveis latentes tal que $z_i\in\mathbb{R}$ e que cada uma delas está relacionada com uma amostra do conjunto de treinamento ${\cal X}$. Dado o conjunto de parâmetros $\boldsymbol{\theta}$, temos que a função de verossimilhança é dada por:
...
\end{enumerate}
No entanto, uma maneira melhor de inicializar $\mu_k$ e $\Sigma_k$ é via algoritmo do $k$-Médias, em que $k=1,2,\ldots,K$. Já os pesos $w_k$ podem ser inicializados como $w_k = \frac{m_k}{|{\cal X}|}$, em que $m_k$ corresponde ao número de amostras do agrupamento $k$.
}
...
\begin{equation}
	\boldsymbol{\Sigma}_k^{(t+1)} =\frac{1}{m_k}\sum_{i=1}^{m_k}T^{(t)}_{ki}(\boldsymbol{x}_i-\boldsymbol{\mu}_k)^T(\boldsymbol{x}_i-\boldsymbol{\mu}_k),
\end{equation}
e
\begin{equation}
	w_k^{(t+1)} = \frac{m_k}{m}.
