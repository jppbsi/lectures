...
\Sli{
\justifying Seja ${\cal Z}=\{z_1,z_2,\ldots,z_m\}$ um conjunto de variáveis latentes em que cada uma delas está relacionada com uma amostra do conjunto de treinamento ${\cal X}^1$. Dado o conjunto de parâmetros $\boldsymbol{\theta}$, temos que a função de verossimilhança é dada por:

\begin{equation}
	L(\boldsymbol{\theta}|{\cal X}^1,{\cal Z}) = p({\cal X}^1,{\cal Z}|\boldsymbol{\theta}),
\end{equation}
\justifying em que $p({\cal X}^1,{\cal Z}|\boldsymbol{\theta})$ corresponde à densidade conjunta de ${\cal X}^1$ e ${\cal Z}$. Ademais, o estimador de máxima verossimilhança do vetor de parâmetros $\boldsymbol{\theta}$ é dado, então, pela \textbf{maximização da verossimilhança marginal} (só temos acesso ao ${\cal X}^1$) dos dados observados, ou seja:

\begin{equation}
	L(\boldsymbol{\theta}|{\cal X}^1) = p({\cal X}^1|\boldsymbol{\theta}) = \int p({\cal X}^1,{\cal Z}|\boldsymbol{\theta})\mathrm{d}{\cal Z}.
\end{equation}
...
\begin{enumerate}
	\item \emph{Expectation} (E): calcula o valor esperado do logaritmo da verossimilhança com relação à distribuição condicional de ${\cal Z}$ dado ${\cal X}^1$ utilizando a estimativa atual dos parâmetros $\boldsymbol{\theta}$:
	\begin{equation}
		Q(\boldsymbol{\theta}|\boldsymbol{\theta}^{(t)}) = \mathbb{E}_{{\cal Z}|{\cal X}^1,\boldsymbol{\theta}^{(t)}}[\log L(\boldsymbol{\theta}|{\cal X}^1,{\cal Z})],
	\end{equation}
	...
	\end{equation}
\end{enumerate}
De maneira geral, a ideia do algoritmo E-M é executar os passos 1 e 2 de maneira iterativa até que $|\boldsymbol{\theta}^{(t+1)}-\boldsymbol{\theta}^{(t)}|<\epsilon$. \textbf{A variável latente ${\cal Z}$ nos diz, basicamente, a pertinência de uma dada amostra $\boldsymbol{x}\in{\cal X}^1$ pertencer à um determinado agrupamento (classe).}
}
...
\begin{itemize}
	\item Observações em ${\cal X}^1$ podem ser discretas ou contínuas (ex: podem ser oriundas de distribuições Gaussianas).
...
\begin{enumerate}
	...
	\begin{itemize}
		\item \underline{Passo E:} para cada amostra $\boldsymbol{x}_i\in{\cal X}^1$ associamos uma pontuação $\gamma_{ik}$ que denota a probabilidade dessa amostra pertencer ao agrupamento $k$.
...
\end{enumerate}
No entanto, uma maneira melhor de inicializar $\mu_k$ e $\Sigma_k$ é via algoritmo do $k$-médias, em que $k=1,2,\ldots,K$. Já os pesos $w_k$ podem ser inicializados como $w_k = \frac{C_k}{|{\cal X}^1|}$, em que $C_k$ corresponde ao número de amostras do agrupamento $k$.
}
...
\begin{itemize}
	\item \underline{Passo E:} temos que as pontuações podem ser calculadas como segue:
	\begin{equation}
		\gamma^{(t)}_{ik} = \frac{w_kp(\boldsymbol{x}_i|\boldsymbol{\mu}_k,\boldsymbol{\Sigma}_k)}{\sum_{j=1}^Kw_jp(\boldsymbol{x}_j|\boldsymbol{\mu}_j,\boldsymbol{\Sigma}_j)}.
	\end{equation}
	A equação assim equivale ao preenchimento de uma matrix $\boldsymbol{\gamma}\in\mathbb{R}^{K\times|{\cal X}^1|}$.
\end{itemize}
}
