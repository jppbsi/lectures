...
\begin{equation}
	C(\langle \boldsymbol{x}_i\rangle)=
\begin{cases}
...
e
\begin{equation}
	C(\pi_{\boldsymbol{x}_j}\cdot \langle \boldsymbol{x}_j,\boldsymbol{x}_k\rangle)=\max\{C(\boldsymbol{x}_j),w(\boldsymbol{x}_j,\boldsymbol{x}_k)\}.
\end{equation}
\justify Neste caso, temos que ${\cal S}$ corresponde ao conjunto de protótipos escolhidos pela MST, $\langle \boldsymbol{x}_i\rangle$ denota o caminho trivial composto apenas pelo nó $\boldsymbol{x}_i$, $C(\langle \boldsymbol{x}_i\rangle)$ corresponde ao custo associado à amostra $\boldsymbol{x}_i$ e $w(\boldsymbol{x}_j,\boldsymbol{x}_k)$ denota a distância (peso da aresta) entre os nós $\boldsymbol{x}_j$ e $\boldsymbol{x}_k$, tal que $\boldsymbol{x}_i,\boldsymbol{x}_j,\boldsymbol{x}_k\in{\cal X}^1$. Já o termo $\pi_{\boldsymbol{x}_j}$ corresponde à um caminho que termina na amostra $\boldsymbol{x}_j$.
}
