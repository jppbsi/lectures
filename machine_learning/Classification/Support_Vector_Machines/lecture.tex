...
\justify Desta forma, obtendo-se $\boldsymbol{\alpha}^\ast$ por algum método de otimização matemática, basta utilizamos ele na Equação 9 para obtemos $\boldsymbol{w}$. Já para calcularmos o $b$, precisamos utilizar as condições de KKT, as quais são condições \textbf{necessárias} para que a solução do problema dado pela Equação 15 seja ótima. Essas condições são dadas pelas seguintes restrições:

\begin{equation}
	\alpha_i^\ast(y_i(\boldsymbol{w}\boldsymbol{x}_i+b)-1) = 0,\ \forall i=1,2,\ldots,m.
\end{equation}
}

\Sli{
\justify Note que $y_i(\boldsymbol{w}\boldsymbol{x}_i+b)-1) = 0$ ocorre apenas para as amostras que se encontram \textbf{sobre} os hiperplanos $H_1$ e $H_2$. Assim, $\alpha_i$ pode ser \textbf{não nulo} apenas sobre os elementos $\boldsymbol{x}_i$ que se encontram nesses hiperplanos. Para as demais amostras $\boldsymbol{x}_j$, isto é, aquelas estão estão definidas além das margens, temos que $\alpha_j = 0$.
...
\Sli{
Temos, então, que $b^\ast$ pode ser calculado como segue:
...
\begin{equation}
	b^\ast = \frac{1}{|{\cal S}|}\sum_{\boldsymbol{x}_i\in{\cal S}}\left\{\frac{1}{y_i}-\sum_{\boldsymbol{x}_j\in{\cal S}}\alpha_j^\ast y_j\boldsymbol{x}_j^T\boldsymbol{x}_i\right\}.
\end{equation}
}
