...
\begin{equation}
	\alpha_i^\ast(y_i(\boldsymbol{w}\boldsymbol{x}_i+\theta)-1) = 0,\ \forall i=1,2,\ldots,m.
\end{equation}
}
...
\justify Lembrando que as restrições originais eram dadas por $y_i(\boldsymbol{w}^T\boldsymbol{x}_i+\theta)\geq 1$, $\forall i=1,2\lodts,m$.
}
