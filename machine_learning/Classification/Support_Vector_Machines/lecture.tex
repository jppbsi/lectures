...
\begin{equation}
\frac{\partial L(\boldsymbol{w}, \theta, \boldsymbol{\alpha})}{\partial b} = 0
\end{equation}
e
\begin{equation}
...
\Sli{
\justify Note que $y_i(\boldsymbol{w}\boldsymbol{x}_i+\theta)-1) = 0$ ocorre apenas para as amostras que se encontram \textbf{sobre} os hiperplanos $H_1$ e $H_2$. Assim, $\alpha_i$ pode ser \textbf{não nulo} apenas sobre os elementos $\boldsymbol{x}_i$ que se encontram nesses hiperplanos. Para as demais amostras $\boldsymbol{x}_j$, isto é, aquelas estão estão definidas além das margens, temos que $\alpha_j = 0$.
...
\begin{equation}
	b^\ast = \frac{1}{|{\cal S}|}\sum_{\boldsymbol{x}_i\in{\cal S}}\left\{\frac{1}{y_i}-\boldsymbol{w}^T\boldsymbol{x}_i\right\},
\end{equation}
em que ${\cal S}\subseteq {\cal X}^1$ denota o conjunto dos vetores de suporte. Podemos, ainda, escrever a equação acima substituindo $\boldsymbol{w}$ pela Equação 9, isto é, em função dos multiplicadores de Lagrange:
...
\begin{equation}
	\epsilon_i^\ast = \max\left\{0,1-\left[y_i\sum_{j=1}^my_j\alpha_j^\ast\boldsymbol{x}_i^T\boldsymbol{x}_j\right]+\theta^\ast\right\}.
\end{equation}
...
Caso $\alpha_i < C$, temos que $\epsilon_i^\ast = 0$ para a Equação 24 ser satisfeita. Ademais, pela Equação 23, concluímos que $\boldsymbol{x}_i$ é um vetor de suporte que está sobre a margem $H_1$ ou $H_2$ (depende de sua classe). Este seria o mesmo caso das SVMs com margens rígidas.
}
...
\begin{enumerate}
	...
	\item $\epsilon_i^\ast = 0$: amostras sobre as margens.
\end{enumerate}
\vspace{-0.7cm}
\begin{center}
