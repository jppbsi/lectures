...
\Sli{
Vamos, agora, analisar as funções de decisão para cada um dos casos apresentados anteriormente:

\begin{itemize}
	\item \underline{Caso 1:} quando as matrizes de covariância das classes são iguais e diagonais, temos um classificador linear cuja função de decisão é dada pela Equação 28:
 
	\begin{equation}
	d_i(\boldsymbol{x}) = w_{i1}^T\boldsymbol{x}+w_{i0},
	\end{equation}
em que $w_{i1} = \frac{1}{\sigma^2}\boldsymbol{\mu_i}$ e $w_{i0}=-\frac{1}{2\sigma^2}\boldsymbol{\mu_i}^T\boldsymbol{\mu_i}+\log p(\omega_i)$.
\end{itemize}
Suponha um problema com duas classes, ou seja, temos que calcular $d_1(\boldsymbol{x})$ e $d_2(\boldsymbol{x})$.
}

\Sli{
Para obtermos a decisão de separação, basta igualarmos $d_1(\boldsymbol{x})$ e $d_2(\boldsymbol{x})$ e encontrarmos esse hiperplano analiticamente, como segue:
...
\Sli{
\begin{itemize}
	\item \underline{Caso 2:} quando as matrizes de covariância das classes são iguais mas não diagonais, temos um classificador linear cuja função de decisão é dada pela Equação 30:

\begin{equation}\nonumber
	d_i(\boldsymbol{x}) = w_{i1}^T\boldsymbol{x}+w_{i0},
\end{equation}
em que $w_{i1} = \boldsymbol{\mu}_i^T\boldsymbol{\Sigma}^{-1}$ e $w_{i0}=-\frac{1}{2}\boldsymbol{\mu}_i^T\Sigma^{-1}\boldsymbol{\mu_i}+\log p(\omega_i)$.
\end{itemize}
...
\Sli{
\begin{itemize}
	\item \underline{Caso 3:} quando existem matrizes de covariância para cada classe, temos um classificador quadrático cuja função de decisão é dada pela Equação 32:
