...
\Sli{
\justifying Seja $\bm{M}^1\in\mathbb{R}^n$ tal que:

\begin{equation}
	\bm{M}^1_i = \frac{1}{m_1}\sum_{j=1}^{m_1}K(\bm{x}_i,\bm{x}_j),
\end{equation}
em que $\bm{x}_j\in\omega_1$ e $\bm{x}_i\in{\cal X}_1$. Basicamente, a equação acima calcula a média da $i$-ésima linha da matriz de \emph{kernel} da classe $1$. O mesmo vale para o cálculo de $\bm{M}^2\in\mathbb{R}^n$, ou seja:
...
\begin{align}\nonumber
	...
	&= \sum_{i=1}^m\alpha_i\underbrace{\frac{1}{m_1}\sum_{j=1}^{m_1}K(\bm{x}_i^T,\bm{x}_j)}_{\bm{M}^1_i}\\\nonumber
...
\begin{align}\nonumber
	...
	&= \sum_{i=1}^m\alpha_i\underbrace{\frac{1}{m_2}\sum_{j=1}^{m_2}K(\bm{x}_i^T,\bm{x}_j)}_{\bm{M}^2_i}\\\nonumber
...
\begin{align}\nonumber
	...
	&= (\bm{\alpha}^T\bm{k}^1-\bm{\alpha}^T\bm{k}^2)({\bm{M}^1}^T\bm{\alpha}-{\bm{M}^2}^T\bm{\alpha})\\\nonumber
