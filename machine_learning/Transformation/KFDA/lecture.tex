...
\Sli{
\justifying Temos que a projeção de $\bm{\mu}^\phi_j$ na direção do vetor $\bm{w}$ é dada por:

\begin{align}\nonumber
	\bm{w}^T\bm{\mu}^\phi_j &= \overbrace{\left(\sum_{i=1}^m\alpha_i\phi(\bm{x}_i)\right)^T}^{\bm{w}^T}\overbrace{\left(\frac{1}{m_j}\sum_{k=1}^{m_j}\phi(\bm{x}_k)\right)}^{\bm{\mu}^\phi_j}=\frac{1}{m_j}\sum_{i=1}^m\sum_{k=1}^{m_j}\alpha_i\overbrace{\phi(\bm{x}_i)^T\phi(\bm{x}_k)}^{\text{kernel}} \\
	 &= \frac{1}{m_j}\sum_{i=1}^m\sum_{k=1}^{m_j}\alpha_i K(\bm{x}_i^T,\bm{x}_k),\ i\in\{1,2\}.
\end{align}
