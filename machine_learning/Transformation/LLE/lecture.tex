...
\begin{equation}
	\hat{\bm{x}}_i \approx \sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_{ij}\bm{x}_j,
\end{equation}
...
\begin{align}\nonumber
	E(\bm{w},\bm{x}_i) &= \norm{\bm{x}_i-\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j\bm{x}_j}^2 = \norm{\cancelto{1\text{ (restrição)}}{\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j}\bm{x}_i-\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j\bm{x}_j}^2 \\
	&= \norm{\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j(\bm{x}_i-\bm{x}_j)}^2 = \sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}\sum_{\bm{x}_k\in{\cal N}(\bm{x}_i)}w_jw_k(\bm{x}_i-\bm{x}_j)^T(\bm{x}_i-\bm{x}_k).
\end{align}
}
...
\end{equation}
Lembrando que o intuito principal da restrição $\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j=1$ é para adicionar propriedades de \textbf{invariância à translação}, isto é, adicionando uma constante $\bm{c}\in\mathbb{R}^n$ ao vetor $\bm{x}_i$ e aos seus vizinhos, o erro de reconstrução não muda.
}
...
\begin{equation}
	\tilde{E}(\bm{w},\bm{x}_i) = \norm{\tilde{\bm{x}_i}-\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j\tilde{\bm{x}_j}}^2 = \norm{\bm{x}_i+\bm{c}-\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j(\bm{x}_j+\bm{c})}^2.
\end{equation}
Podemos reescrever esta equação como segue:

\begin{align}\nonumber
	\tilde{E}(\bm{w},\bm{x}_i) &= \norm{\tilde{\bm{x}_i}-\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j\tilde{\bm{x}_j}}^2 = \norm{\bm{x}_i+\bm{c}-\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j(\bm{x}_j+\bm{c})}^2\\
	&= \norm{\bm{x}_i+\bm{c}-\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j\bm{x}_j-\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j\bm{c}}^2\\\nonumber
	&= \norm{\bm{x}_i+\bm{c}-\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j\bm{x}_j-\bm{c}\cancelto{1}{\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j}}^2\\\nonumber
	&= \norm{\bm{x}_i+\bm{c}-\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j\bm{x}_j-\bm{c}}^2 = \norm{\bm{x}_i-\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j\bm{x}_j}^2 = E(\bm{w},\bm{x}_i)\nonumber.
\end{align}
Assim, a translação não afeta o erro quando temos a restrição $\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j=1$.
}
...
\begin{equation}
	E(\bm{w}) = \norm{\bm{x}_i-\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j\bm{x}_j}^2+\alpha\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_j^2.
\end{equation}
}
...
\begin{equation}
\begin{split}
	\Phi(\hat{\bm{X}}) &= \sum_{i=1}^m\left[\hat{\bm{x}}_i^T\hat{\bm{x}}_i-\hat{\bm{x}}_i^T\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_{ij}\hat{\bm{x}}_j-\left(\sum_jw_{ij}\hat{\bm{x}}_j\right)^T\hat{\bm{x}}_i+\right.\\
...
\begin{equation}
\begin{split}
	\Phi(\hat{\bm{X}}) &= \sum_{i=1}^m\hat{\bm{x}}_i^T\hat{\bm{x}}_i-\sum_{i=1}^m\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_{ij}\hat{\bm{x}}_i^T\hat{\bm{x}}_j-\sum_{i=1}^m\sum_{\bm{x}_j\in{\cal N}(\bm{x}_i)}w_{ji}\hat{\bm{x}}_j^T\hat{\bm{x}}_i+\\
...
\State \Call{LLE}{$\bm{X}, k, d$}
	...
	\State{Normalize os pesos $\bm{w}_i$ para que $\sum_j\bm{w}_{ij} = 0$.}
	\State{Construa a matriz $\bm{W}\in\mathbb{R}^{m\times m}$ cujas linhas são os pesos estimados $\bm{w}_i$.}
	...
	\State{Selecione os $d$ autovetores não nulos associados aos $d$ menores autovalores para definir a matriz $\hat{\bm{X}}\in\mathbb{R}^{m\times d}$, em que cada coluna é um autovetor.}
	\EndFor
\end{algorithmic}
}
