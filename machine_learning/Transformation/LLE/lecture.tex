...
\Sli{
\justifying Vamos, agora, definir a matriz $\bm{C}^i\in\mathbb{R}^{n\times n}$:

\begin{equation}
	C^i_{jk} = (\bm{x}_i-\bm{x}_j)^T(\bm{x}_i-\bm{x}_k).
\end{equation}
Desta forma, podemos reescrever a expressão para o erro local de reconstrução (Equação 4) como segue:

\begin{equation}
	E(\bm{w},\bm{x}_i) = \sum_j\sum_kw_jC^i_{jk}w_k = \bm{w}^T\bm{C}^i\bm{w}.
\end{equation}
