...
\Sli{
A Equação 2 é usualmente chamada de \textbf{função de custo} ou \textbf{função de perda}. Podemos simplificar a notação escrevendo a Equação 2 da seguinte forma:
...
\begin{equation}\nonumber
\begin{split}
J(w_1) &= \frac{1}{2m}\sum_{i=1}^m(h_{\boldsymbol{w}}(x_i)-y_i)^2\\
&= \frac{1}{2m}\sum_{i=1}^m(w_1x_i-y_i)^2\\
&= \frac{1}{2m}\sum_{i=1}^m(x_i-y_i)^2\\
&= \frac{1}{2.3}\sum_{i=1}^3(x_i-y_i)^2\\
...
\end{equation}
em que $j\in\{0,1\}$, $\alpha$ corresponde à \textbf{taxa de aprendizado} e $t$ ao número da iteração. Já o termo $\frac{\partial J(\boldsymbol{w})}{\partial w_j}$ denota a \textbf{derivada parcial}, que deverá ser calculada para cada parâmetro de nossa equação da reta.
}
...
\justify Suponha, agora, que tenhamos mais variáveis para representar o problema, visto que utilizar apenas o tamanho da casa pode não ser suficiente para estimarmos o seu valor (número de quartos, capacidade da garagem, piscina, etc).\newline

\justify Neste caso, assuma que $n$ corresponda ao número dessas variáveis, ou seja, temos agora que $\boldsymbol{x}_i\in\mathbb{R}^n$. Assim sendo, nossa função hipótese possui a seguinte formulação:

\begin{align}
	h_{\boldsymbol{w}}(\boldsymbol{x})&=w_0+w_1x^1+w_2x^2+\ldots w_nx^n\nonumber\\
	&=w_0+\sum_{j=1}^nw_jx^j.
\end{align}
} 

\Sli{
Para fins de simplicidade, vamos assumir que $x^0=1$, ou seja, $\boldsymbol{x}=[x^0\ x^1\ x^2\ \ldots\ x^n]\in\mathbb{R}^{n+1}$. Assim sendo, a Equação 10 pode ser escrita da seguinte forma:

\begin{align}
	h_{\boldsymbol{w}}(\boldsymbol{x}) &= w_0x^0+\sum_{j=1}^nw_jx^j\nonumber\\
	&= \boldsymbol{w}^T\boldsymbol{x}.
\end{align}
A equação acima é, na verdade, a equação do hiperplano, ou seja, uma reta multimensional. Desta forma, a regressão linear multivariada \textbf{generaliza} a regressão linear univariada para um número maior de dimensões.
}
...
\begin{itemize}
	...
	\item Função de custo: $J(\boldsymbol{w})=\frac{1}{2m}\sum_{i=1}^m(h_{\boldsymbol{w}}(\boldsymbol{x}_i)-y_i)^2$.
	\item Derivada parcial: $\frac{\partial J(\boldsymbol{w})}{\partial w_j}= \frac{1}{m}\sum_{i=1}^m[(h_{\boldsymbol{w}}(x_i)-y_i)x_i^j]$.
\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Pode convergir mais rápido do que o gradiente descendente (pode não avaliar todo o espaço de busca).
...
\SliT{Regularização}{
Com o intuito de evitar com que nossos modelos tornem-se muito ``especializados", podemos fazer uso de técnicas de \textbf{regularização}. Vamos visualizar as três situações que podem ocorrer quando estamos trabalhando com regressão de dados.

\begin{tabular}{ccc}
	\includegraphics[scale=0.157]{./figs/Regressao_Linear_Fig25.png} &
	\includegraphics[scale=0.157]{./figs/Regressao_Linear_Fig26.png} &
	\includegraphics[scale=0.157]{./figs/Regressao_Linear_Fig27.png}\\
	Subtreinamento & Bom treinamento & Sobretreinamento\\
\end{tabular}
}
...
\begin{center}
\begin{tabular}{cc}
	\includegraphics[scale=0.197]{./figs/Regressao_Linear_Fig28.png} &\hspace{1.5cm}
	\includegraphics[scale=0.197]{./figs/Regressao_Linear_Fig29.png} \\
 $h_{\boldsymbol{w}}(x) = w_0+w_1x+w_2x^2$ & \hspace{1.5cm}$h_{\boldsymbol{w}}(x) = w_0+w_1x+w_2x^2+w_3x^3+w_4x^4$\\
\end{tabular}
\end{center}
}

\Sli{
\justify Como podemos minimizar o efeito dos pesos associados aos graus maiores? Uma solução para penalizá-los seria multiplicá-los por valores altos. Vamos modificar a Equação 2, que corresponde ao erro médio quadrático:

\begin{equation}
	\boldsymbol{w}^\ast = \argmin_{\boldsymbol{w}}\left\{\frac{1}{2m}\sum_{i=1}^m(h_w(x_i)-y_i)^2+\overbrace{500w_3}^{w_3\approx 0}+\underbrace{800w_4}_{w_4\approx 0}\right\}.
\end{equation}

\justify Como o objetivo é minimizar a função de custo, a tendência é que o gradiente descendente atribua valores muito pequenos para $w_3$ e $w_4$ no intuito de diminuir a sua influência no valor final. Assim, a regularização visa, de maneira geral, atribuir valores pequenos para os elementos em $\boldsymbol{w}$ de tal forma a termos funções ``mais simples".
}

\Sli{
Desta forma, temos uma nova função de custo por erro médio quadrático, que agora adiciona um \textbf{termo de regularização} à Equação 4:

\begin{align}
\label{e.Regularized_cost_function}
J(\boldsymbol{w}) &= \frac{1}{2m}\left[\sum_{i=1}^m(h_w(x_i)-y_i)^2+\lambda\sum_{j=1}^nw^2_j\right]\nonumber\\
&= \frac{1}{2m}\sum_{i=1}^m(h_w(x_i)-y_i)^2+\frac{\lambda}{2m}\sum_{j=1}^nw^2_j,
\end{align}
em que $\lambda$ corresponde ao parâmetro de regularização, o qual controla a relação \textbf{custo-benefício} entre não regularização e sobretreinamento. Lembrando que a regularização não se aplica à $w_0$.
}
...
\begin{center}
\begin{tabular}{ccc}
	\includegraphics[scale=0.197]{./figs/Regressao_Linear_Fig30.png} &
	\includegraphics[scale=0.197]{./figs/Regressao_Linear_Fig31.png} &
	\includegraphics[scale=0.197]{./figs/Regressao_Linear_Fig32.png} \\
	$\lambda = 0$ & $0<\lambda<\infty$ & $\lambda \rightarrow \infty$\\
	Sem regularização & Algum valor apropriado & $h_{\boldsymbol{w}}(x) = w_0$
\end{tabular}
\end{center}
}

\Sli{
As derivadas parciais para $\frac{\partial J(\boldsymbol{w})}{\partial w_j}$ dadas pelas Equações 8 e 9 agora podem ser formuladas da seguinte maneira:
\begin{equation}
	\frac{\partial J(\boldsymbol{w})}{\partial w_0} = \frac{1}{m}\sum_{i=1}^m(h_{\boldsymbol{w}}(x_i)-y_i),
\end{equation}
e
\begin{equation}
	\frac{\partial J(\boldsymbol{w})}{\partial w_j} = \frac{1}{m}\sum_{i=1}^m[(h_{\boldsymbol{w}}(x_i)-y_i)x_i^j]+\frac{\lambda}{m}w_j,\ \forall j\in\{1,2,\ldots,n\}.
\end{equation}
